var LotteryView = Backbone.View.extend({
    ...
    lottery_play_types: {
        ...
        '30x5':[{code:'101',name:"单式投注"},{code:'102',name:"复式投注"},{code:'103',name:"胆拖投注"}]
    },
    //获取彩种名称
    ...
    timeTick: function () {
        ...
        if (lag > 0 && this.lotteryType != 'k3') {
            ...
            //if (this.lotteryType == 'k3') {
                ...
                $('#lcenter_round').text(l.number);
                var endTime = (new Date(l.stopTime)).getTime();
                var lag = (endTime - now.getTime() + 90000) / 1000;
                var second = Math.floor(lag % 60);
                var minite = Math.floor((lag / 60) % 60);
                var day = Math.floor((lag / 3600) / 24);
                var hour = day * 24 + Math.floor((lag / 3600) % 24);
    ...
    ticketChange: function (model, options) {
        ...
        if (money > 0) {
            if (money >20000) {
                $("#myModal .modal-content").html('<div class="pop-all text-center"><div class="jycl-h2">对不起，您的投注金额超限，单次投注总金额不得超过20000元。</div><div class="pop-button text-center"><button class="btn btn-mg btn-inverse fg-white close_modal">确 定</button></div></div>');
                $('#myModal').modal('show');
                this.reset_ticket();
                return;
            }
            ...
            if ((this.lotteryType == 'ssq' && this.playType == '101') || (this.lotteryType == '307' && this.playType == '101')||(this.lotteryType=='30x5'&&this.playType=='101')
    ...
    addTicket: function (item) {
        ...
        if ((this.lotteryType == 'ssq' && this.playType == '101') || (this.lotteryType == '307' && this.playType == '101')||(this.lotteryType=='30x5'&&this.playType=='101')
            || (this.lotteryType == '3d' && (this.playType == '201' || this.playType == '202' || this.playType == '203'))
            || (this.lotteryType == '22x5' && (this.playType == '101' || this.playType == '104' || this.playType == '106' || this.playType == '109' || this.playType == '112'))
            || (this.lotteryType == 'k3' && (this.playType == '101' || this.playType == '102' || this.playType == '105' || this.playType == '103' || this.playType == '104' || this.playType == '108' || this.playType == '106' || this.playType == '107'))) {
            $('#balls .on-redball').removeClass('on-redball');
            var ob = {'beishu': 1, 'zhuihao': 1, 'money': 0, 'nums': {}};
            this.ticket.set(ob);
        }
    ...
    selectMul: function (e) {
        ...
        if(this.lotteryType=='ssq'||this.lotteryType=='k3'||(this.lotteryType=='3d'&&(this.playType=='207'||this.playType=='208'||this.playType=='209'||this.playType=='212'||this.playType=='214'))||(this.lotteryType=='307'&&this.playType=='103')){
    ...
    renderPlayType: function (playType) {
        ...
        if ((this.lotteryType == 'ssq' && playType == '101') || (this.lotteryType == '307' && playType == '101')||(this.lotteryType=='30x5'&& playType=='101')
        	...
        	if(this.lotteryType =='ssq'){	
	            var ssq_rule = {
	            	"1":"双色球投注去分为红色球号码区和蓝色号码区，红色球号码区由1-33共三十三个号码组成，蓝色球号码区由1-16共十六个号码组成。投注时选择6个红色球号码和1个蓝色球号码组成一注进行单式投注，每注金额人民币2元。",
	            	"2":"所选号码个数超过单式投注的号码个数，所选号码可组合为每一种单式投注方式的多注彩票的投注。 ",
	            	"3":"先选取少于单式投注号码个数的号码作为胆码（即每注彩票均包含的号码），再选取除胆码以外的号码作为拖码 "
	            };
	            var theType = playType.substr(-1, 1);
                $("#rule_content").text(ssq_rule[theType]);
            //福彩3D规则
        	}if(this.lotteryType == '3d'){
        		 var sd_rule = {
	            	"201":"投注号码与当期开奖号码按位全部相同（百位+十位+个位），即中奖，单注奖金固定为1040元 。",
	            	"202":"将三个号码的所有排列方式作为一注投注号码进行投注。如果一注组选的三个号码中有两个号码相同，则包括三种不同的排列方式，称为“组选3”，单注奖金固定为346元。",
	            	"203":"将三个号码的所有排列方式作为一注投注号码进行投注。如果一注组选的三个号码各不相同，则包括六种不同的排列方式，称为“组选6”，单注奖金固定为173元。",
	            	"204":"所选号码个数超过单式投注的号码个数，所选号码可排列或组合为每一种单式投注方式的多注彩票的投注。 ",
	            	"205":"对0～9中最少选择2位进行投注。",
	            	"206":"对0～9中最少选择4位进行投注。 ",
	            	"207":"对1～27中选择一位进行投注。",
	            	"208":"同时用单选和组选的方式对三个号码进行投注。如果三个号码中有两个号码相同，则包括三种不同的排列方式，称为“包选3”。全中：单注奖金固定为693元；组中：单注奖金固定为173元。",
	            	"209":"同时用单选和组选的方式对三个号码进行投注。如果三个号码各不相同，则包括六种不同的排列方式，称为“包选6”。全中：单注奖金固定为606元；组中：单注奖金固定为86元。",
	            	"212":"对1～26中选择一位进行投注。",
	            	"214":"对百位、十位或个位中任意两个位置上的号码进行投注。"
	            };
	            var theType = playType;
                $("#rule_content").text(sd_rule[theType]);
            //七乐彩规则
        	}if(this.lotteryType == '307'){
        		 var qlc_rule = {
	            	"1":"从01—30共30个号码中选择7个号码组合为一注投注号码。每注金额人民币2元。",
	            	"2":"从30个号码中选择8－16个号码，将每7个号码的组合方式都作为一注投注号码的多注投注。 ",
	            	"3":"在30个号码中选择1至6个号码作为每注都有的胆码，再补充其它不同的号码，进行每7个号码为一组的多注投注。 "
	            };
	            var theType = playType;
                $("#rule_content").text(qlc_rule[theType]);
            //30选5规则
        	}if(this.lotteryType == '30x5'){
        		 var ssx5_rule = {
	            	"1":"从1-30共三十个自然数中任取五个号码组合为一注进行单式投注。每注金额人民币2元。 ",
	            	"2":"所选号码个数超过单式投注的号码个数，所选号码可组合为每一种单式投注方式的多注彩票的投注。",
	            	"3":"先选取少于单式投注号码个数的号码作为胆码（即每注彩票均包含的号码），再选取除胆码以外的号码作为拖码，胆码与拖码个数之和必须多于单式投注号码个数，由胆码与拖码的每一种组合按单式投注方式组成多注彩票的投注。 "
	            };
	            var theType = playType.substr(-1, 1);
                $("#rule_content").text(ssx5_rule[theType]);
        	}
        ...
        this.ticket.set(ob);
        //console.log('注码格式', this.ticket,ob);
        this.empty_cart();
    ...
    buy_lottery: function (e) {
        ...
        if (tickets.length > 0) {
            ...
            this.logger.info('=====buy_lottery===', JSON.stringify(info));
            pay_view.inti(info);
        } else {
    ...
    bqjixuan5: function () {
        ...
        for (var c = 0; c < 5 - count; c++) {
            ...
            if (this.lotteryType == 'ssq' && this.playType == '101') {
                ...
                oj.set({'nums': nums});
            } else if (this.lotteryType == '30x5' && this.playType == '101') {
                var balls = this.getRandomBalls(5, 1, 30);
                var nums = {red: this.getBallStr(balls)};
                oj.set({'nums': nums});
                ...
                var b1 = this.getRandomBalls(2, 0, 9);
                oj.set({nums: {hao1: [b1[0]], hao2: [b1[0]], hao3: [b1[1]]}});
